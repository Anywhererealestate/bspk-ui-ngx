...
function writeComponentMetaFiles(components: ComponentMeta[]) {
    ...
    components.forEach((component) => {
        ...
        const hasOverrideBasicUsage = hasSettings?.includes('basicUsage');

        const componentsToGenerate: { code: string; name: string; selector: string; imports: string[] }[] = [];

        // only generate the basic usage component if there is an example in the JSDoc and there isn't already a static example component
        if (!hasOverrideBasicUsage && basicUsage) {
...
function componentSettings() {
    ...
    output
        ...
        .map((file) => {
            ...
            componentSettings[slug] = fs.readFileSync(file, 'utf-8');
        })
        .sort();
