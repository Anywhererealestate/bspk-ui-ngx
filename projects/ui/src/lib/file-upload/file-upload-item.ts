    ...
    <div data-bspk="file-upload-item" [attr.data-status]="status()">
      <div data-main style="display:flex; justify-content:space-between; gap:var(--spacing-sizing-03);">
        <div data-name style="min-width:0;">
          ...
          <div *ngIf="sizeLabel" data-size style="opacity:0.7;">{{ sizeLabel }}</div>
        </div>
      ...
      <div *ngIf="status() === 'error' && errorMessage()" data-error style="margin-top:var(--spacing-sizing-02);">
        {{ errorMessage() }}
      </div>
    </div>
  `,
...
export class UIFileUploadItem {
  ...
  readonly sizeBytes = input<number | undefined>(undefined);
