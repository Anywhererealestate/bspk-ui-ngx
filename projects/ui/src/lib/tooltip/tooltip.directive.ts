...
export class TooltipDirective implements OnDestroy {
  ...
  private normalizedConfig(): Required<TooltipProps> {
    const base: TooltipProps =
      typeof this.tooltip === 'string' ? { label: this.tooltip } : this.tooltip || {};
  ...
  show() {
    ...
    const tooltipRef: ComponentRef<Tooltip> | undefined = this.overlayRef?.attach(
      new ComponentPortal(Tooltip),
    );
