...
export class TooltipDirective implements OnDestroy {
    ...
    @Input('tooltip') tooltip: TooltipConfig | string = '';
    ...
    private normalizedConfig(): Required<TooltipConfig> {
        const base: TooltipConfig = typeof this.tooltip === 'string' ? { label: this.tooltip } : this.tooltip || {};

        return {
            label: base.label ?? '',
            placement: (base.placement as any) ?? base.placement ?? 'top',
            showTail: base.showTail ?? true,
            disabled: coerceBooleanProperty(base.disabled) ?? false,
        };
    }
